...
class ActivateTender implements ShouldQueue
{
    use Dispatchable, 
        InteractsWithQueue, 
        Queueable, 
    ...
    {
        if($this->tender->status->type !== ActiveStatus::DEFAULT_SLUG) {
            $this->tender->setAsActive();
            event(new TenderActivation($this->tender, $this->tender->user));
        }
