...
@section('card-header')
    <x-form-title>Створити групу мета-полів</x-form-title>
                ...
                @foreach($postTypesModel->get() as $postType)
                    <li class="mb-1">
                        <input 
                            ...
                            @checked((bool) old("accept_post_types[{$postType->slug}]") == $postType->id)
                        >
                        <label 
                ...
                @foreach($taxonomyModel->get() as $taxonomy)
                    <li class="mb-1">
                        <input 
                            ...
                            @checked((bool) old("accept_taxonomies[{$taxonomy->slug}]") == $taxonomy->id)
                        >
                        <label 
                            class="custom-control-label" 
