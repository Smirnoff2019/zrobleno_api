...
$(document).ready(function () {
  ...
  function onEvent(event, selector) {
  ...
  function onClick() {
    ...
    onEvent.apply(void 0, ['click'].concat(args));
  }
  ...
  function onChange() {
    ...
    onEvent.apply(void 0, ['change'].concat(args));
  }
  ...
  (function ($) {
    ...
    var app = {
      env: env,
      apiRoute: function apiRoute(routeName) {
    ...
    var action = {
      ...
      loading: function loading() {
        var type = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'loading';
      ...
      hide: function hide() {
        var type = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'hide';
        return $('.action-box[action]').attr('action', type);
      }
    };

    function ModalGallery(target) {
      ...
      var gallery = {
        ...
        loadContent: function loadContent() {
          ...
          $.ajax({
            ...
            success: function success(res) {
              ...
              _this.gallery.append($(res.data.body));

              if (res.data.images.next_page_url) {
                _this.curent_url = res.data.images.next_page_url;

                _this.actionBox.show();
              } else {
        ...
        getData: function getData() {
          var formData = new FormData(this.gallery[0]);
          var selectedImages = formData.getAll(this.galleryItemName);
          return mode === 'single' ? selectedImages[0] : selectedImages;
        },
        submitBtn: {
          ...
          counter: function counter(count) {
            return this.btn.find('span').text(count);
          },
          hideCounter: function hideCounter() {
        ...
        onShow: function onShow(callback) {
          if (typeof callBack == 'function') this.hooks.onShow.push(callBack);
        },
        onHide: function onHide(callBack) {
          if (typeof callBack == 'function') this.hooks.onHide.push(callBack);
        },
        onHidden: function onHidden(callBack) {
          if (typeof callBack == 'function') this.hooks.onHidden.push(callBack);
        },
        onSubmit: function onSubmit(callBack) {
          if (typeof callBack == 'function') this.hooks.onSubmit.push(callBack);
        }
      };
      ...
      gallery.submitBtn.btn.on('click', function () {
        var _this2 = this;
        ...
        gallery.hooks.onSubmit.forEach(function (callback) {
          callback.apply(void 0, [gallery, gallery.getData(), $(_this2)].concat(args));
        });
      });
      gallery.modal.on('show.bs.modal', function () {
        ...
        gallery.gallery.on('click', function () {
          ...
          var selectedImages = formData.getAll('g_image[]');
        ...
        gallery.hooks.onShow.forEach(function (callback) {
          callback.apply(void 0, [gallery].concat(args));
        });
        ...
        gallery.hooks.onHide.forEach(function (callback) {
          callback.apply(void 0, [gallery, gallery.getData()].concat(args));
        });
        ...
        gallery.gallery.html(' ');
        gallery.hooks.onHidden.forEach(function (callback) {
          callback.apply(void 0, [gallery, gallery.getData()].concat(args));
        });
    ...
    function handleImagesGalleryWidgetModal(button) {
      button.on('click', function () {
        var Gallery = ModalGallery($(this).data('target'), $(this).data('mode') || 'single');
        Gallery.curent_url = app.apiRoute('images');
        var input = $(this).data('input');
        var storage = $(this).parent().find(input);
        var previews = $(this).parent().find(".preview-images");
        var mode = $(this).data('mode');
        Gallery.onSubmit(function (gl, data, btn) {
          var res = Gallery.getData();
          ...
          if (mode === 'single') {
            ...
            storage.val(_data.id);
            previews.removeClass('empty');
            previews.append($("<img src=\"".concat(_data.file.url, "\" data-id=\"").concat(_data.id, "\" class=\"img-thumbnail m-1 shadow-sm\">")));
          } else {
            previews.html('');

            var _data2 = res.map(function (item) {
              item = JSON.parse(item);
              previews.removeClass('empty');
              previews.append($("<img src=\"".concat(item.file.url, "\" data-id=\"").concat(item.id, "\" class=\"img-thumbnail m-1 shadow-sm\">")));
              return item.id;
            });

            storage.val(JSON.stringify(_data2));
          }
        });
      });
    }
    ...
    (function () {
      ...
      $('#upload-next-gallery-content-btn').on('click', function () {
        var button = $(this);
        var url = button.data('next-page-url');
    ...
    (function () {
      onChange('#upload-image-form input', function (item, e) {
        $('#upload-image-form').submit();
      });
    })();

    (function () {
      onClick('#input-image-delete-btn', function (item, e) {
    ...
    function initSlugable() {
      ...
      $('[data-slugable]').each(function (key, item) {
        slugableInput($(item));
      });
    ...
    (function () {// const editorsAreas = $('.ckeditor');
    ...
    (function () {
      var controlSelector = '.meta-field-card-collapse--control';
      var $controls = $("#meta-fields-schema ".concat(controlSelector));
      var elems = $("[data-toggle=\"change-sync\"]");

      function handleCollapseControl(controller) {
        var $control = $(controller);
        var $input = $(controller).find('input[type="checkbox"]');
        var $header = $control.closest('.meta-field-card-header');
        var $cardName = $header.find('.meta-field-card-header--action');
        var $collapse = $($(controller).data('target'));

        function isActive() {
          return $header.find("".concat(controlSelector)).filter(function (key, item) {
            var result = $(item).attr('aria-expanded') == "true";
      ...
      function initSyncChanges(elems) {
        ...
        $('.meta-field-param-field--type').on('change', function () {
          ...
          var type = $(this).val();
          var value = trans[type];

          if (value) {
            syncChanges($(this), value);
            var id = $(this).closest('.meta-field-card').find('input#_data-field').val();
            var parent_id = $(this).closest('.meta-field-card').find('input#_data-parent-field').val();
            var body = $(this).closest('.meta-field-card-body--params');
            var header = $(body.closest('.meta-field-card').find('.meta-field-card-header').get(0));
            var url = "".concat(env.base_url).concat(env.api.metaField);
            var data = {
              id: id,
              parent_id: parent_id,
            ...
            getMetaFieldByType(data, url, function (res, data) {
              var card = $(res);
              $(_this3).closest('.meta-field-card').replaceWith(card);
              var $controls = card.find("".concat(controlSelector));
              var $elems = card.find("[data-toggle=\"change-sync\"]");
      ...
      function getMetaFieldByType(data, url) {
        var callback = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : function () {};
        $.ajax({
          url: url,
          data: data,
      ...
      function removeMetaFieldCardHandler(elem) {
        elem.on('click', function () {
          var check = confirm('Вы уверены что хотите удалить это мета поле?');
      ...
      function appendMetaFieldCardHandler(elem) {
        elem.on('click', function (e) {
          ...
          e.preventDefault(); // const url = `${env.base_url}${env.api.metaField}`;

          var parent_id = $(this).closest('.meta-field-card').find('input#_data-field').val(); // const optionsValue = $(this).closest('.meta-field-card').find('.meta-field-param-field').find('[data-param-name="options"]').val();
          ...
          getMetaFieldByType(data, url, function (res, data) {
            var card = $(res);
            card.insertBefore($(_this4).closest('.meta-fields-schema-footer'));
            var $controls = card.find("".concat(controlSelector));
            var $elems = card.find("[data-toggle=\"change-sync\"]");
            sortableMetaFieldCards(card.find(".sortable"));
            removeMetaFieldCardHandler(card.find('.meta-field-card--delete'));
        ...
        elem.sortable({
          ...
          update: function update(event, ui) {
            if (ui.sender) {
              ...
              var cards = sender.find(">.meta-field-card");
            ...
            if (ui.item) {
              ...
              var _cards = field.parent().find(">.meta-field-card");
          ...
          stop: function stop(event, ui) {
            var $card = $(ui.item);
            var isChild = $card.parent().hasClass('meta-field-children-list');
            var $inputParentId = $card.find('input#_data-parent-field');
            $card.removeClass('shadow');

            if (isChild) {
              var parentId = $card.parent().data('card');
              $inputParentId.val(parentId);
            } else {
              $inputParentId.val('');
            }
          },
          connectWith: "#meta-fields-schema .sortable" // dropOnEmpty: false,
      ...
      function appendNewFieldHandler($area, data, url) {
        $.ajax({
          ...
          success: function success(res) {
            var $field = $(res);
            var button = $field.find('button[data-toggle="modal"][data-type="image-gallery"]');
      ...
      onClick('.append-new-contractor-portfolio-image', function (item) {
        var area = $(item).data('append-area');
        var action = $(item).data('append-action');
        var method = $(item).data('append-method');
        var data = $(item).data('append-data');
        var $area;
        console.log(data);
        ...
        appendNewFieldHandler($area, data, action);
      });
      onClick('#append-new-contractor-portfolio', function (item) {
        var $area = $(item).closest('.tab-pane').find('#accordion');
        ...
        $.ajax({
          ...
          success: function success(res) {
            var $card = $(res);
            var button = $card.find('button[data-toggle="modal"][data-type="image-gallery"]');
            button.data('target', '#modal-galery-for-meta');
            handleImagesGalleryWidgetModal(button);
            $area.append($card);
          }
    ...
    (function () {
      ...
      $('.meta-field--images-gallery .delete-image-from-gallery').on('click', function () {
        var result = confirm('Вы уверены что хотите удалить это поле?');
        var gallery = $(this).closest('.meta-field--images-gallery');

        if (result) {
          var _gallery = $(this).closest('.images-gallery-item').remove();
        }

        var items = gallery.find('tbody .images-gallery-item');
      ...
      $('.meta-field--images-gallery .append-image-to-gallery').on('click', function (e) {
        ...
        $.ajax({
          ...
          success: function success(res) {
            var metaField = $(res);
            var gallery = btn.closest('.meta-field--images-gallery');
    ...
    (function () {
      var deleteActions = $('.action-cell--delete-record');
      var buttons = deleteActions.find('button');
      buttons.on('click', function (e) {
        e.preventDefault();
        var result = confirm('Ви впевнені, що хочете видали цей запис назавжди?');
    ...
    (function () {
      ...
      onClick('#create-new-images-group-btn', function (elem, e) {
        var btn = $(elem);
      });
    })();
  })(jQuery);
...
__webpack_require__(/*! ./portfolio */ "./resources/js/portfolio.js");
...
$(document).ready(function () {
  (function () {
    var portfolioCollapse = $("#form-append-new-contractor-portfolio").css("display", "none");
    var portfolioCreateButton = $("#append-new-contractor-portfolio");
...
/***/ 0:
/*!********************************************************************************************!*\
  !*** multi ./resources/js/app.js ./resources/sass/app.scss ./resources/sass/document.scss ***!
  \********************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

__webpack_require__(/*! C:\OpenServer\Projects\zrobleno\api\resources\js\app.js */"./resources/js/app.js");
__webpack_require__(/*! C:\OpenServer\Projects\zrobleno\api\resources\sass\app.scss */"./resources/sass/app.scss");
module.exports = __webpack_require__(/*! C:\OpenServer\Projects\zrobleno\api\resources\sass\document.scss */"./resources/sass/document.scss");
